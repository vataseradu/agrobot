	...
	{#if !loading}
		<div in:fade={{ duration: 50 }} class="w-full h-full flex flex-col">
			...
			{:else}
				...
				<div
					class="absolute top-0 left-0 w-full h-full bg-cover bg-center bg-no-repeat"
					style="background-image: url({bgUrl}); opacity: 0.12;  "
				/>

				<div
					class="absolute top-0 left-0 w-full h-full bg-linear-to-t from-white to-white/95 dark:from-gray-900 dark:to-gray-900/97 z-0"
				/>
			{/if}
